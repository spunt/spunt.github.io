function scrubTimeseries(SDthresh, SAVEIT, PLOTIT)
% DVARS (D referring to temporal derivative of timecourses, VARS referring
% to RMS variance over voxels) indexes the rate of change of BOLD signal
% across the entire brain at each frame of data. To calculate DVARS, the
% volumetric timeseries is differentiated (by backwards differences) and
% RMS signal change is calculated over the whole brain. DVARS is thus a
% measure of how much the intensity of a brain image changes in comparison
% to the previous timepoint (as opposed to the global signal, which is the
% average value of a brain image at a timepoint).
if nargin < 1, SDthresh = 3; end
if nargin < 2, SAVEIT = 1; end
if nargin < 3, PLOTIT = 1; end
img = uigetvol('Select 4D Image to Process');
fprintf('\n4D Image:  %s\n', img);
...
rp = load(rpfile);
zdvars = abs(oneoutzscore(dvars));
badIDX = find(any(zdvars>SDthresh, 2));
badTRs = zeros(length(zdvars),length(badIDX));
for i = 1:length(badIDX)
    badTRs(badIDX(i),i) = 1;
end
all = [rp badTRs];
if SAVEIT
    [p,n] = fileparts(rpfile);
    outname = fullfile(p, ['dvars_' n '.txt']);
    save(outname, 'all', '-ascii');
    fprintf('\nSaved nuisance regressor file to:  %s\n', outname);
end
if PLOTIT
    rp(:,4:6) = rp(:,4:6)*57.3;
    figure('Color','white', 'units', 'normal', 'position', [.25 .25 .5 .5]);
    subplot(2,1,1);
    plot(rp, 'LineWidth', 2); hold on;
    set(get(gca,'YLabel'), 'String', 'Movement (mm/degrees)', 'FontSize', 15);
    set(get(gca,'XLabel'), 'String', 'Scan', 'FontSize', 13);
    legendh = legend({'x' 'y' 'z' 'pitch' 'roll' 'yaw'});
    set(legendh, 'Orientation', 'vertical', 'Location', 'NorthWest', 'FontSize', 12);box off
    subplot(2,1,2);
    plot(dvars, 'LineWidth', 3);
    hold on; plot(badTRs*max(dvars), 'linewidth', 2, 'color', [0 0 0]);
    title(gca, sprintf('Identified %d timepoints for scrubbing', length(badIDX)), 'FontSize', 18, 'FontWeight', 'bold');
    set(get(gca,'YLabel'), 'String', 'DVARS', 'FontSize', 15);
    set(get(gca,'XLabel'), 'String', 'Scan', 'FontSize', 15);box off
end
end
% =========================================================================
% SUBFUNCTIONS
...
%
%  NIFTI data format can be found on: https://nifti.nimh.nih.gov
%
