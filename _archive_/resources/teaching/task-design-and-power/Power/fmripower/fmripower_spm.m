...
c6 = [.987 .988 .992];
c7 = [0.9903    0.9903    0.9903];
framecolortitle = [0.2265    0.2265    0.2265];
bgcolor = [0.0893    0.0893    0.0893];
fs1 = 16; 
fs2 = 16;
fs3 = 22;
...
f = figure('units', 'pixels','Visible','on',...
	   'Position',size_win, 'MenuBar', 'none', ...
	 'Resize','off' , 'Name', 'fMRIpower-SPM', ...
	   'NumberTitle', 'off', 'color', bgcolor, ...
	   'HandleVisibility', 'off');
   
M1 = uimenu(f, 'Label', 'Font Size', 'Separator', 'on');
M2 = uimenu(M1, 'Label', 'Increase', 'Accelerator', 'i', 'Callback', {@menu_changefontsize, f});
M3 = uimenu(M1, 'Label', 'Decrease', 'Separator', 'on', 'Accelerator', 'd', 'Callback', {@menu_changefontsize, f});
%---------------------------
%---------------------------
tmp=0;
size_b1=[10, 350, 480, 150];
s1_box=uicontrol(f, 'Style', 'frame', 'Position', size_b1, 'backgroundcolor', c7);

s1_box_text=uicontrol(f, 'Style', 'text', 'string', 'Set SPM options',...
		      'Position', [20, size_b1(2)+size_b1(4)-10, 190,20], ...
		      'FontWeight', 'bold', 'Fontsize', fs2, ...
		      'backgroundcolor', framecolortitle, 'foreg', [1 1 1]);
%---------------------
gap=(size_b1(4)-150)/4;
...
gfeat_text=uicontrol(f, 'Style', 'text', 'String', 'Design setup',...
		     'Position',  [20,lev3+30,200,20],...
		      'FontWeight', 'bold',...
		      'FontSize', fs1, 'backgroundcolor', c7, ...
...
s2_box=uicontrol(f, 'Style', 'frame', 'Position', size_b2, 'backg', c7);
...
design_box=uicontrol(f,'Style','edit','String','',...
          'Position',[20,lev3_2,300,30], 'BackgroundColor', [1,1,1],...
...
design_push=uicontrol(f,'Style','pushbutton','String',...
          ...
          'Position',[335,lev3_2,140,30], 'backgroundcolor', [1,1,1]); 
%------------------------------
...
function menu_changefontsize(varargin)
    ...
    h = findall(f, 'type', 'uicontrol');
    fs = get(h, 'fontsize');
    for i = 1:length(h)
        set(h(i), 'fontsize', fs{i}*Fsc);
    end
end
