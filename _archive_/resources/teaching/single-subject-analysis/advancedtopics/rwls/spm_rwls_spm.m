function [SPM] = spm_rwls_spm(SPM)
% [Re]ML Estimation of a General Linear Model
% FORMAT [SPM] = spm_rwls_spm(SPM)
...
%                        for RWLS the threshold is removed and ALL voxels
%                        in the brain are used for stable estimation
%
...
%                           ----------------
%
% The following SPM.fields are set by spm_rwls_spm (unless specified)
%
%     xVi.V      - estimated non-sphericity trace(V) = rank(V)
%     xVi.h      - hyperparameters  xVi.V = xVi.h(1)*xVi.Vi{1}. For rwls
%                  these are the weighting factors for each image
...
%                           ----------------
%
%     ResStats.s - sum of spatially whitened residuals over the volumne
%     ResStats.ss - sums of squares of spatially whitened residual over each volume
%     ResStats.n - number of estimated voxels per volumne
%     ResStats.v - Variance estimate: ss/n
%
%                           ----------------
%
...
% Andrew Holmes, Jean-Baptiste Poline & Joern Diedrichsn & Karl Friston
% $Id: spm_rwls_spm.m 3468 2009-10-15 18:59:38Z karl $
...
for z = 1:nbz:zdim                       %-loop over planes (2D or 3D data)
    ...
    %RWLS
    CTempS  = [];           %-current residual sum (averaged over voxels within scan)
    CTempSS = [];           %-current residual SS  (averaged over voxels within scan)
    %RWLS END
    ...
    for bch = 1:nbch                     %-loop over blocks
        ...
        if CrS
            ...
            else            % Final estimation: Store away residual time series stats
                % [RWLS
                % Store away Spatially whitend residuals
                q=size(res,2);
                trRV   = spm_SpUtil('trRV',xX.xKXs);
    ...
    % RWLS
    %- Calculate the suffient statistic for mean and variance
    %- To report the statistics on the residuals for that slice
    %------------------------------------------------------------------
    if ~isempty(Q)
        TEMPSTATS.N(z)=length(Q);
        TEMPSTATS.SS(:,z)=nansum(CTempSS,2);
        TEMPSTATS.S(:,z)=nansum(CTempS,2);
    else
        TEMPSTATS.N(:,z)=0;
        TEMPSTATS.SS(:,z)=zeros(nScan,1);
        TEMPSTATS.S(:,z)=zeros(nScan,1);
    end;
    % RWLS END
...
% %-Saves the residuals images and applies logtransform
% %==========================================================================
% resimg = files([SPM.swd filesep 'ResI_*img']);
% bob_spm_logtransform(resimg,1);
...
% ----------------------------------------------------------
SPM.ResStats.s=sum(TEMPSTATS.S,2);
SPM.ResStats.ss=sum(TEMPSTATS.SS,2);
SPM.ResStats.n=sum(TEMPSTATS.N,2);
% END RWLS
